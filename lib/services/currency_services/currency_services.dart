...
class CurrencyService {
  final String _baseUrl =
      'https://api.frankfurter.app/latest?from=EUR&to=TRY,USD,GBP,AED,JPY,AUD,DKK,CHF,SEK,CAD,KWD,NOK,SAR,BGN,RON,RUB,CNY,PKR,QAR,ZAR,AZN';
    ...
    if (response.statusCode == 200) {
      ...
      rates.forEach((currency, rate) {
        if (currency != 'TRY' && currencyNames.containsKey(currency)) {
          ...
          currencyRates.add(
            CurrencyRate.fromMap(currency, currencyNames[currency]!, buyingRate / unitFactor, sellingRate / unitFactor),
          );
        }
      });
